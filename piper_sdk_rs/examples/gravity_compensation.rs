...
//!
//! Prerequisites:
//!   - URDF model file (e.g., piper_no_gripper_description.urdf)
...
impl GravityCompensationCalculator {
    ...
    pub fn compute_torques(&mut self, angles_rad: &[f64; 6], velocities_rad: &[f64; 6]) -> Vec<f64> {
        ...
        let qfrc_bias = self.data.qfrc_bias();
        let gravity_torques: Vec<f64> = qfrc_bias.iter()
            .take(6)  // Only take the first 6 joints
            .copied()
            .collect();
...
fn main() -> Result<()> {
    ...
    let loop_rate = Duration::from_millis(20); // 50 Hz control loop
    let mut iteration = 0;
    ...
    while running.load(Ordering::SeqCst) {
        ...
        if let Ok(Some(joint_state)) = piper.get_joint_state() {
            ...
            let angles_rad = [
                joint_state.angles[0].to_radians() as f64,
                joint_state.angles[1].to_radians() as f64,
                joint_state.angles[2].to_radians() as f64,
                joint_state.angles[3].to_radians() as f64,
                joint_state.angles[4].to_radians() as f64,
                joint_state.angles[5].to_radians() as f64,
            ];

            // Joint velocities (currently JointState doesn't provide velocities)
            // TODO: Add velocity feedback from joint_state or motor feedback
            let velocities_rad = [0.0; 6];

            // Compute gravity compensation torques using MuJoCo
            let torques_vec = gravity_calc.compute_torques(&angles_rad, &velocities_rad);
            
            // Convert to array for processing
            let torques: [f32; 6] = [
                torques_vec[0] as f32,
                torques_vec[1] as f32,
                torques_vec[2] as f32,
                torques_vec[3] as f32,
                torques_vec[4] as f32,
                torques_vec[5] as f32,
            ];

            let torques_reduced: [f32; 6] = [
                torques[0] * MOTOR_REDUCE_FACTORS[0],
                torques[1] * MOTOR_REDUCE_FACTORS[1],
                torques[2] * MOTOR_REDUCE_FACTORS[2],
                torques[3] * MOTOR_REDUCE_FACTORS[3],
                torques[4] * MOTOR_REDUCE_FACTORS[4],
                torques[5] * MOTOR_REDUCE_FACTORS[5],
            ];
            ...
            // Print status every 10 iterations (0.2 seconds)
            if iteration % 10 == 0 {
                println!(
    ...
    // Send MIT control commands for each joint to set torque to zero with 0.4 damping for 5 seconds
    let shutdown_start = std::time::Instant::now();
    println!("\n\nApplying zero torque commands for safe shutdown... waiting 5 seconds.");
    while shutdown_start.elapsed() < Duration::from_secs(5) {    
        piper.enable_mit_mode(true)?;
        for motor_num in 1..=6 {
            let mit_ctrl = JointMitControl::new(
                motor_num,
            ...
            // Note: In a real implementation, we would need access to `piper` here.
            // This is a simplified example; consider using channels or other means
            // to send commands to the main thread for shutdown.
            piper.send_joint_mit_control(&mit_ctrl)?;
        }
